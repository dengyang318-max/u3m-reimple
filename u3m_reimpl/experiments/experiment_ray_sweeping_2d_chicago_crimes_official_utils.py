...
"""
Unified experiment script for Chicago Crimes dataset with configurable algorithm variants.
...
def main(argv=None):
    """Run ray-sweeping experiment on Chicago Crimes dataset with configurable variants."""
    ...
    # Setup logging
    log_file = (run_dir / "chicago_crimes_official_utils_log.md").open("w", encoding="utf-8")
    ...
    x_train_new, x_train_new_prime, targets, _ = build_point_sets_from_data(
    ...
    for ax, pts, title in zip(axes, [x_train_new, x_train_new_prime], 
                               ["Primary points (Lon, Lat)", "Rotated points [max_lat - Lat, Lon]"]):
        ax.scatter(pts[:, 0], pts[:, 1], c=targets if targets is not None else None, s=10)
        ax.set_title(title)
        ax.set_xlabel("Lon" if "Primary" in title else "x'")
        ax.set_ylabel("Lat" if "Primary" in title else "y'")
    plt.tight_layout()
