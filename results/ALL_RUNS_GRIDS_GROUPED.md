# College Admission Official-Utils Runs (Grouped)

This document merges run details with the grouping/analysis. Groups are ordered by parameter impact (baseline -> vector_transfer off -> L2).

## Parameter impact (recap)
- Vector transfer (`disable_vector_transfer`): main driver of differences; turning it off changes found tails/directions.
- Normalization (`use_l2_norm`): secondary; small drifts only.
- Min-shift (`disable_min_shift`): negligible alone; minor interaction when vector_transfer is off.
- Data structure (`use_linkedlist`): negligible (tiny numeric drift).
- Initialization (`use_first_intersection_init`): negligible on its own.

## Group A 鈥?Baseline (vector_transfer ON, L1)
> Percentile tables nearly identical; toggling linkedlist/min_shift/first_init causes only tiny numeric drift.

### 20251208_205408
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205408

![20251208_205408 top grid](20251208_205408/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.254 |    1.247 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.222 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.700 |    0.375 |    1.128 |
|       0.20 |    0.675 |    0.409 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.740 |    0.350 |    1.041 |
|       0.20 |    0.738 |    0.364 |    1.286 |
|       0.10 |    0.750 |    0.286 |    1.222 |
|       0.08 |    0.812 |    0.250 |    1.133 |
|       0.04 |    0.875 |    0.500 |    1.286 |

### 20251208_205427
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: True
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205427

![20251208_205427 top grid](20251208_205427/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.254 |    1.247 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.222 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.700 |    0.375 |    1.128 |
|       0.20 |    0.675 |    0.409 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.740 |    0.350 |    1.041 |
|       0.20 |    0.738 |    0.364 |    1.286 |
|       0.10 |    0.750 |    0.286 |    1.222 |
|       0.08 |    0.812 |    0.250 |    1.133 |
|       0.04 |    0.875 |    0.500 |    1.286 |

### 20251208_205445
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: False
- `disable_min_shift`: True
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205445

![20251208_205445 top grid](20251208_205445/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.254 |    1.247 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.222 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.700 |    0.375 |    1.128 |
|       0.20 |    0.675 |    0.409 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.740 |    0.350 |    1.041 |
|       0.20 |    0.738 |    0.364 |    1.286 |
|       0.10 |    0.750 |    0.286 |    1.222 |
|       0.08 |    0.812 |    0.250 |    1.133 |
|       0.04 |    0.875 |    0.500 |    1.286 |

### 20251208_205538
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205538

![20251208_205538 top grid](20251208_205538/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.690 |    0.367 |    1.128 |
|       0.20 |    0.675 |    0.435 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.277 |    1.222 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.353 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.690 |    0.295 |    1.174 |
|       0.20 |    0.662 |    0.308 |    1.000 |
|       0.10 |    0.675 |    0.381 |    1.000 |
|       0.08 |    0.656 |    0.353 |    1.133 |
|       0.04 |    0.625 |    0.400 |    0.600 |

### 20251208_205557
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: True
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205557

![20251208_205557 top grid](20251208_205557/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.690 |    0.367 |    1.128 |
|       0.20 |    0.675 |    0.435 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.277 |    1.222 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.353 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.690 |    0.295 |    1.174 |
|       0.20 |    0.662 |    0.308 |    1.000 |
|       0.10 |    0.675 |    0.381 |    1.000 |
|       0.08 |    0.656 |    0.353 |    1.133 |
|       0.04 |    0.625 |    0.400 |    0.600 |

### 20251208_205619
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: False
- `disable_min_shift`: True
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205619

![20251208_205619 top grid](20251208_205619/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.690 |    0.367 |    1.128 |
|       0.20 |    0.675 |    0.435 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.277 |    1.222 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.353 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.690 |    0.295 |    1.174 |
|       0.20 |    0.662 |    0.308 |    1.000 |
|       0.10 |    0.675 |    0.381 |    1.000 |
|       0.08 |    0.656 |    0.353 |    1.133 |
|       0.04 |    0.625 |    0.400 |    0.600 |


## Group B 鈥?Vector-transfer OFF (biggest change)
> Turning off vector_transfer shifts tail metrics noticeably; this is the dominant behavioral change.

### 20251208_205502
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: True
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205502

![20251208_205502 top grid](20251208_205502/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.646 |    0.407 |    1.276 |
|       0.20 |    0.613 |    0.492 |    0.905 |
|       0.10 |    0.650 |    0.562 |    1.105 |
|       0.08 |    0.625 |    0.571 |    1.000 |
|       0.04 |    0.562 |    0.533 |    1.286 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.640 |    0.400 |    1.198 |
|       0.20 |    0.650 |    0.481 |    0.905 |
|       0.10 |    0.575 |    0.452 |    0.739 |
|       0.08 |    0.625 |    0.538 |    0.684 |
|       0.04 |    0.583 |    0.545 |    1.400 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.307 |    1.090 |
|       0.50 |    0.794 |    0.128 |    0.990 |
|       0.20 |    0.850 |    0.000 |    1.222 |
|       0.10 |    0.825 |    0.000 |    1.000 |
|       0.08 |    0.812 |    0.000 |    1.133 |
|       0.04 |    0.812 |    0.000 |    0.600 |

### 20251208_205521
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: True
- `disable_min_shift`: True
- `disable_vector_transfer`: True
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205521

![20251208_205521 top grid](20251208_205521/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.640 |    0.400 |    1.174 |
|       0.20 |    0.650 |    0.481 |    0.905 |
|       0.10 |    0.575 |    0.452 |    0.739 |
|       0.08 |    0.625 |    0.538 |    0.684 |
|       0.04 |    0.625 |    0.625 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.650 |    0.407 |    1.273 |
|       0.20 |    0.620 |    0.483 |    0.927 |
|       0.10 |    0.650 |    0.562 |    1.105 |
|       0.08 |    0.625 |    0.571 |    1.000 |
|       0.04 |    0.562 |    0.533 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.638 |    0.400 |    1.261 |
|       0.20 |    0.650 |    0.517 |    0.951 |
|       0.10 |    0.590 |    0.529 |    0.696 |
|       0.08 |    0.562 |    0.500 |    0.882 |
|       0.04 |    0.533 |    0.462 |    1.500 |

### 20251208_205636
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: True
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205636

![20251208_205636 top grid](20251208_205636/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.646 |    0.407 |    1.276 |
|       0.20 |    0.613 |    0.492 |    0.905 |
|       0.10 |    0.650 |    0.562 |    1.105 |
|       0.08 |    0.625 |    0.571 |    1.000 |
|       0.04 |    0.562 |    0.533 |    1.286 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.640 |    0.400 |    1.198 |
|       0.20 |    0.650 |    0.481 |    0.905 |
|       0.10 |    0.575 |    0.452 |    0.739 |
|       0.08 |    0.625 |    0.538 |    0.684 |
|       0.04 |    0.583 |    0.545 |    1.400 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.307 |    1.090 |
|       0.50 |    0.794 |    0.128 |    0.990 |
|       0.20 |    0.850 |    0.000 |    1.222 |
|       0.10 |    0.825 |    0.000 |    1.000 |
|       0.08 |    0.812 |    0.000 |    1.133 |
|       0.04 |    0.812 |    0.000 |    0.600 |

### 20251208_205653
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: True
- `disable_min_shift`: True
- `disable_vector_transfer`: True
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205653

![20251208_205653 top grid](20251208_205653/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.646 |    0.407 |    1.276 |
|       0.20 |    0.613 |    0.492 |    0.905 |
|       0.10 |    0.650 |    0.562 |    1.105 |
|       0.08 |    0.625 |    0.571 |    1.000 |
|       0.04 |    0.562 |    0.533 |    1.286 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.640 |    0.400 |    1.198 |
|       0.20 |    0.650 |    0.481 |    0.905 |
|       0.10 |    0.575 |    0.452 |    0.739 |
|       0.08 |    0.625 |    0.538 |    0.684 |
|       0.04 |    0.583 |    0.545 |    1.400 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.307 |    1.090 |
|       0.50 |    0.794 |    0.128 |    0.990 |
|       0.20 |    0.850 |    0.000 |    1.222 |
|       0.10 |    0.825 |    0.000 |    1.000 |
|       0.08 |    0.812 |    0.000 |    1.133 |
|       0.04 |    0.812 |    0.000 |    0.600 |

### 20251208_205816
**Parameters:**
- `use_official_style`: False
- `use_linkedlist`: False
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: False
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205816

![20251208_205816 top grid](20251208_205816/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.640 |    0.400 |    1.174 |
|       0.20 |    0.650 |    0.481 |    0.905 |
|       0.10 |    0.575 |    0.452 |    0.739 |
|       0.08 |    0.625 |    0.538 |    0.684 |
|       0.04 |    0.625 |    0.625 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.650 |    0.407 |    1.273 |
|       0.20 |    0.620 |    0.483 |    0.927 |
|       0.10 |    0.650 |    0.562 |    1.105 |
|       0.08 |    0.625 |    0.571 |    1.000 |
|       0.04 |    0.562 |    0.533 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.111 |
|       0.50 |    0.638 |    0.400 |    1.261 |
|       0.20 |    0.650 |    0.517 |    0.951 |
|       0.10 |    0.590 |    0.529 |    0.696 |
|       0.08 |    0.562 |    0.500 |    0.882 |
|       0.04 |    0.533 |    0.462 |    1.500 |


## Group C 鈥?L2 normalization (vector_transfer ON)
> L2 vs. L1 causes small, consistent drifts; much smaller impact than disabling vector_transfer.

### 20251208_205710
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: True
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205710

![20251208_205710 top grid](20251208_205710/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.254 |    1.247 |
|       0.20 |    0.775 |    0.250 |    1.105 |
|       0.10 |    0.775 |    0.000 |    1.222 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.700 |    0.375 |    1.128 |
|       0.20 |    0.675 |    0.409 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.740 |    0.350 |    1.041 |
|       0.20 |    0.738 |    0.364 |    1.286 |
|       0.10 |    0.750 |    0.286 |    1.222 |
|       0.08 |    0.812 |    0.250 |    1.133 |
|       0.04 |    0.875 |    0.500 |    1.286 |

### 20251208_205727
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: False
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: True
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205727

![20251208_205727 top grid](20251208_205727/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.690 |    0.367 |    1.128 |
|       0.20 |    0.675 |    0.435 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.277 |    1.222 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.353 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.690 |    0.295 |    1.174 |
|       0.20 |    0.662 |    0.308 |    1.000 |
|       0.10 |    0.675 |    0.381 |    1.000 |
|       0.08 |    0.656 |    0.353 |    1.133 |
|       0.04 |    0.625 |    0.400 |    0.600 |

### 20251208_205744
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: False
- `use_first_intersection_init`: True
- `disable_min_shift`: False
- `disable_vector_transfer`: False
- `use_l2_norm`: True
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205744

![20251208_205744 top grid](20251208_205744/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.254 |    1.247 |
|       0.20 |    0.775 |    0.250 |    1.105 |
|       0.10 |    0.775 |    0.000 |    1.222 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.000 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.700 |    0.375 |    1.128 |
|       0.20 |    0.675 |    0.409 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.740 |    0.350 |    1.041 |
|       0.20 |    0.738 |    0.364 |    1.286 |
|       0.10 |    0.750 |    0.286 |    1.222 |
|       0.08 |    0.812 |    0.250 |    1.133 |
|       0.04 |    0.875 |    0.500 |    1.286 |

### 20251208_205759
**Parameters:**
- `use_official_style`: True
- `use_linkedlist`: True
- `use_first_intersection_init`: True
- `disable_min_shift`: True
- `disable_vector_transfer`: False
- `use_l2_norm`: True
- `top_k`: 10
- `min_angle_step`: 0.314
- `n_samples`: None
- `excel_path`: None
- `timestamp`: 20251208_205759

![20251208_205759 top grid](20251208_205759/official_utils_top_grid.png)

**Top-1 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.714 |    0.321 |    1.111 |
|       0.50 |    0.690 |    0.367 |    1.128 |
|       0.20 |    0.675 |    0.435 |    0.818 |
|       0.10 |    0.641 |    0.364 |    2.000 |
|       0.08 |    0.677 |    0.444 |    1.818 |
|       0.04 |    0.800 |    0.400 |    1.500 |

**Top-2 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.765 |    0.277 |    1.222 |
|       0.20 |    0.775 |    0.308 |    1.162 |
|       0.10 |    0.775 |    0.000 |    1.353 |
|       0.08 |    0.844 |    0.000 |    1.133 |
|       0.04 |    0.875 |    0.000 |    1.286 |

**Top-3 percentile table:**
| Percentile | Accuracy | F1-score | F/M ratio |
|-----------:|---------:|---------:|----------:|
|       1.00 |    0.712 |    0.320 |    1.100 |
|       0.50 |    0.690 |    0.295 |    1.174 |
|       0.20 |    0.662 |    0.308 |    1.000 |
|       0.10 |    0.675 |    0.381 |    1.000 |
|       0.08 |    0.656 |    0.353 |    1.133 |
|       0.04 |    0.625 |    0.400 |    0.600 |

