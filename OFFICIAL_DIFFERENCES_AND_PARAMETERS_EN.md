...
|---------------------|------------------------|----------------------------|----------------------------------|-------------------|
| **Normalization Method** | L1 normalization (`v / sum(v)`) | L2 normalization (`v / ||v||_2`) | Configurable L1/L2 | `use_l1_norm` |
| **Polar Angle Calculation** | `atan(y/x)` | `atan2(y, x)` | `atan(y/x)` (fixed) | None (fixed to match official) |
...
| **Termination Condition** | Exact check (`x == 0`) | Tolerance check (`x < 1e-10`) | Exact check (`x == 0`) | None (fixed to match official) |
| **vector_transfer** | Supported (for rotated point sets) | Not supported | Configurable | `enable_vector_transfer` |
| **Intersection Enumeration Strategy** | Enumerate all → Sort → Filter first quadrant | Filter first quadrant during enumeration | Enumerate all → Sort → Filter first quadrant | None (fixed to match official) |
| **Median Update Sorting** | `atan(y/x)` | `atan2(y, x)` | `atan(y/x)` (fixed) | None (fixed to match official) |
